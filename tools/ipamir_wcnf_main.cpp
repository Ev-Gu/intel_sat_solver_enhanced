// Feed a WCNF file to IntelSat through the IPAMIR API (batch load + solve).
// Based on Yevgeny's ipamir_fuzzer_main.cpp (feature/ipamir-incremental-maxsat).
// Not a fuzzer — use scripts/compare_wcnf_batch_vs_ipamir.sh to compare this
// path with the normal batch executable (-M 1) on the same .wcnf file.
...
static void print_solution(int status, void* solver, int32_t print_max_var)
{
